...
To reduce the blocking time of the first paint, Qwik can inline the critical styles for the current view. This means that the browser will not need to download the styles for the current view. To tell Qwik to inline styles, just add the `?inline` parameter to the import.
...
Please note that you need to import the styles using the `?inline` query parameter.
